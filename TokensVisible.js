...
tokensVisible.pushTokenBackListener = function (event) {
  if (event.target !== document.body && event.target !== canvas.app.view)
    return;
  ...
  if (event.key == tokensVisible.castRayshotkey) {
    switch (tokensVisible.currentCastRaysmode) {
      case "Standard":
        tokensVisible.setProperCastRays("Enhanced");
        break;
      case "Enhanced":
        tokensVisible.setProperCastRays("Super");
        break;
      case "Super":
  ...
  if (event.key == tokensVisible.sightCachehotkey) {
    switch (tokensVisible.currentSightCacheMode) {
      case "On":
        tokensVisible.setProperSightCache("Off");
        break;
      case "Off":
        tokensVisible.setProperSightCache("On");
        break;
      default:
        tokensVisible.setProperSightCache("On");
    }
...
tokensVisible.hoverToken.hook = Hooks.on(
  "controlToken",
  (token, controlled) => {
    if (controlled) {
...
Hooks.once("canvasReady", () => {
  tokensVisible.setProperCastRays(
    game.settings.get("TokensVisible", "castRays")
  );
  tokensVisible.setProperSightCache(
    game.settings.get("TokensVisible", "sightCache")
  );
});
...
tokensVisible.canvasInitializeSources = function () {
  ...
  else canvas.initializeSources(); // Foundry 0.7.9
};
...
tokensVisible.setProperCastRays = function (value) {
  switch (value) {
    case "Standard":
      ...
      break;
    case "Enhanced":
      tokensVisible._setEnhancedCastRays();
      break;
    case "Super":
      tokensVisible._setExtraEnhancedCastRays();
      break;
    default:
      tokensVisible._setStandardCastRays();
      value = "Standard";
  ...
  if (game.ready) {
    ui.notifications.info(
      game.i18n.localize("TOKENSVISIBLE.castRays") + " : " + value
    );
...
tokensVisible.setProperSightCache = function (value) {
  switch (value) {
    case "On":
      tokensVisible.enableTurboSight();
      break;
    case "Off":
  ...
  if (game.ready) {
    ui.notifications.info(
      game.i18n.localize("TOKENSVISIBLE.sightCache") +
        " : " +
        game.i18n.localize("TOKENSVISIBLE.sightCache" + value)
    );
  }
};
...
tokensVisible.setupRenderColors = function () {
  if (tokensVisible.defaultcolors["toggleActiveFG"] == undefined)
    tokensVisible.defaultcolors["toggleActiveFG"] = $(
      "li.control-tool.active.toggle"
    ).css("color");
  if (tokensVisible.defaultcolors["toggleActiveBG"] == undefined)
    tokensVisible.defaultcolors["toggleActiveBG"] = $(
      "li.control-tool.active.toggle"
    ).css("background-color");
  if (tokensVisible.defaultcolors["activeFG"] == undefined)
    tokensVisible.defaultcolors["activeFG"] = $(
      "li.control-tool.active:not(.toggle)"
    ).css("color");
  if (tokensVisible.defaultcolors["activeBG"] == undefined)
    tokensVisible.defaultcolors["activeBG"] = $(
      "li.control-tool.active:not(.toggle)"
    ).css("background-color");
  ...
  setColor(
    "activeBG",
    "#controls .control-tool.active:not(.toggle)",
    "background-color"
  );
  ...
  function setColor(settingName, jQuerySpec, cssProperty) {
    ...
    if (colorValue && !nameToHexA(colorValue).endsWith("00")) {
      document
        .querySelectorAll(jQuerySpec)
        .forEach((e) => e.style.setProperty(cssProperty, colorValue));
    } else
      document
        .querySelectorAll(jQuerySpec)
        .forEach((e) =>
          e.style.setProperty(
            cssProperty,
            tokensVisible.defaultcolors[settingName]
          )
        );
  }
};
...
Hooks.once("ready", () => {
  ...
  function lightwrapper(wrapped, ...args) {
    if (
      this.data.hidden &&
      this.emitsLight &&
      game.settings.get("TokensVisible", "hiddenCanLight") == "Yes"
    ) {
...
function tokenHasSight(tokenInQuestion) {
  if (
    tokenInQuestion.data.hidden &&
    game.settings.get("TokensVisible", "hiddenCanSee") == "No"
  )
    return false;
...
Hooks.once("setup", () => {
  ...
  tokensVisible.autopanMargin = game.settings.get(
    "TokensVisible",
    "autopanningMargin"
  );
  tokensVisible.wallsCancelAnimation = game.settings.get(
    "TokensVisible",
    "wallsCancelAnimation"
  );
  tokensVisible.castRayshotkey = game.settings.get(
    "TokensVisible",
    "castRayshotkey"
  );
  tokensVisible.sightCachehotkey = game.settings.get(
    "TokensVisible",
    "sightCachehotkey"
  );
  tokensVisible.setupCombatantMasking(
    game.settings.get("TokensVisible", "combatantHidden")
  );
});
...
Hooks.once("init", () => {
  ...
  libWrapper.register(
    ...
    function (wrapped, ...args) {
      ...
      if (
        game.settings.get("TokensVisible", "blindTokensControllable") == "No"
      ) {
  ...
  libWrapper.register(
    ...
    function (wrapped, attributes, { context, name, duration, ontick }) {
      const tokenAnimationSpeed =
        game.settings.get("TokensVisible", "tokenAnimationSpeed") / 10.0;

      if (
        (tokenAnimationSpeed != 1 || tokensVisible.cancelTokenAnimation) &&
        name != undefined
      ) {
        if (
          name.substring(0, 6) == "Token." &&
          name.substring(name.length - 16) == ".animateMovement"
        ) {
          if (tokensVisible.cancelTokenAnimation) duration = 0;
          else duration = duration / tokenAnimationSpeed;
        }
      }
  ...
  function tokenTestPartyPerm(token, permission) {
    ...
    game.users["players"].forEach((f) => {
      console.warn(f);
      partyMemberHasPermission =
        partyMemberHasPermission || tokenTestPerm(token, f, permission);
    });
  ...
  libWrapper.register(
    ...
    function (wrapped) {
      const blindTokensControllable =
        game.settings.get("TokensVisible", "blindTokensControllable") == "Yes";
      if (
        !game.user.isGM &&
        this._controlled &&
        !tokenHasSight(this) &&
        !blindTokensControllable
      )
        this.release();
  ...
  libWrapper.register(
    ...
    function (wrapped) {
      const tokenMultiVision = game.settings.get(
        "TokensVisible",
        "tokenMultiVision"
      );
      ...
      if (
        computerSaysYes &&
        (tokenMultiVision == "Yes" ||
          this.data._id == tokensVisible.lastControlledToken?.data._id ||
          !canvas.effects.visibility.tokenVision)
      )
        return true;

      if (!canvas.effects.visibility.tokenVision || !this.hasSight)
      ...
      if (game.user.isGM) {
        ...
        switch (tokenMultiVision) {
          case "Party":
            return (
              tokenTestPartyPerm(this, "OBSERVER") &&
              (this.data.disposition ==
                tokensVisible.lastControlledToken?.data.disposition ||
                (this.observer &&
                  tokensVisible.lastControlledToken == undefined))
            );
          case "Yes":
          ...
          case "Limited":
            ...
            break;
          case "Never":
            if (this.data._id == tokensVisible.lastControlledToken?.data._id)
              return true;
        }
      }
  ...
  if (!(typeof ClientDatabaseBackend === "undefined")) {
    libWrapper.register(
      moduleName,
      "ClientDatabaseBackend.prototype._updateEmbeddedDocuments",
      async function (
        wrapped,
        documentClass,
        parent,
        { updates, options, pack },
        user
      ) {
    ...
    libWrapper.register(
      moduleName,
      "Entity.prototype.updateEmbeddedEntity",
  ...
  Hooks.on("preUpdateToken", (token, changes, data, ...args) => {
    const tokenAnimationSpeed = game.settings.get(
      "TokensVisible",
      "tokenAnimationSpeed"
    );
    ...
    const ray = new Ray({ x: token.x, y: token.y }, target);
    if (tokensVisible.wallsCancelAnimation == "Always") data.animate = false;
    if (
      tokensVisible.wallsCancelAnimation == "Yes" &&
      token._object.checkCollision(target)
    )
      data.animate = false;

    if (data.animate) {
      setProperty(data, `animation`, {
        movementSpeed: tokenAnimationSpeed,
        duration: 0,
        easing: "linear",
      });
    }
...
tokensVisible.SightLayer = {
  _defaultcastRays:
    WallsLayer.castRays != undefined
      ? WallsLayer.castRays
      : CanvasVisibility_castRays,
  _defaultcomputeSight:
    WallsLayer.prototype.computePolygon != undefined
      ? WallsLayer.prototype.computePolygon
      : CanvasVisibilitycomputeSight,
};
// the conditional assignment is for legacy 0.7.x support
tokensVisible.Combat = {
  _defaultcreateEmbeddedEntity:
    Combat.prototype.createEmbeddedDocuments != undefined
      ? Combat.prototype.createEmbeddedDocuments
      : Combat.prototype.createEmbeddedEntity,
};

tokensVisible.SightLayer._turboComputeSight = function (
  wrapped,
  origin,
  radius,
  { type, angle = 360, density = 6, rotation = 0, unrestricted = false } = {}
) {
  if (type == undefined || (type != undefined && type == "sight")) {
    ...
    let key =
      ...
      origin.x +
      origin.y * 7.654 +
      canvas.dimensions.width * 1.1 +
      canvas.dimensions.height * 3.3 +
      density * 11 +
      (unrestricted ? 33 : -1) +
      angle * 1.1 +
      rotation +
      canvas.walls.endpoints.length * 12.2;
    let sightResult = tokensVisible.SightCache.get(key);
    if (sightResult != undefined) {
      return sightResult;
    }

    sightResult = wrapped(origin, radius, {
      type,
      angle,
      density,
      rotation,
    ...
    if (tokensVisible.SightCache.size > 1000)
      tokensVisible.SightCache.delete(
        tokensVisible.SightCache.keys().next().value
      );

    tokensVisible.SightCache.set(key, sightResult);
    return sightResult;
  } else {
    return wrapped(origin, radius, {
      type,
      angle,
      density,
      rotation,
...
tokensVisible.enableTurboSight = function () {
  ...
  if (WallsLayer.prototype.computePolygon != undefined)
    patch(
      "WallsLayer.prototype.computePolygon",
      tokensVisible.SightLayer._turboComputeSight,
      "MIXED"
    );
  else
    patch(
      "CanvasVisibility.computeSight",
      tokensVisible.SightLayer._turboComputeSight,
      "MIXED"
...
tokensVisible.disableTurboSight = function () {
  ...
  if (WallsLayer.prototype.computePolygon != undefined) {
    unpatch(
      "WallsLayer.prototype.computePolygon",
      tokensVisible.SightLayer._defaultcomputeSight
    );
  } else {
    // legacy 0.7.x support
    unpatch(
...
tokensVisible.SightLayer._DI_castRays = function (
  x,
  y,
  distance,
  { density = 4, endpoints, limitAngle = false, aMin, aMax } = {},
  cast,
  standardArray,
  rayAccuracy,
  rays,
  ...
  if (endpoints.length || standardArray.length) {
    ...
    if (density < Math.min(2, originaldensity))
      density = Math.min(2, originaldensity); // dont want to make the quality too good/expensive. Use Supermode for that.
...
tokensVisible._setEnhancedCastRays = function () {
  ...
  else patch("CanvasVisibility._castRays", enhancedCastRays, "OVERRIDE"); // legacy 0.7.x support

  function enhancedCastRays(
    x,
    y,
    distance,
    { density = 4, endpoints, limitAngle = false, aMin, aMax } = {}
    ...
    let rays = [];
    const rayAccuracy = canvas.effects.visibility.tokenVision
    ...
    rays = tokensVisible.SightLayer._DI_castRays.call(
      this,
      x,
      y,
      distance,
...
tokensVisible._setExtraEnhancedCastRays = function () {
  const stubsorter = (rays) => {
    /* nothing to do because the rays are cast in sorted order in this mode */
  };
  ...
  if (WallsLayer.castRays != undefined) {
    ...
    patch("CanvasVisibility._castRays", superCastRays, "MIXED"); // legacy 0.7.x support
  }

  function superCastRays(
    wrapped,
    x,
    y,
    distance,
    { density, endpoints, limitAngle = false, aMin, aMax } = {}
  ) {
    if (!canvas.effects.visibility.tokenVision)
      return wrapped(x, y, distance, {
        density: density + 3,
        endpoints,
        limitAngle,
        aMin,
    ...
    rays = tokensVisible.SightLayer._DI_castRays.call(
      ...
      y,
      distance,
      { density: Math.min(2.0, density), endpoints, limitAngle, aMin, aMax },
      cast,
      [],
      rayAccuracy,
      rays,
...
tokensVisible._setStandardCastRays = function () {
  ...
  if (WallsLayer.castRays != undefined) {
    ...
    // legacy 0.7.x support
    unpatch(
      "CanvasVisibility._castRays",
      tokensVisible.SightLayer._defaultcastRays
    );
  }
};
  ...
  switch (settingValue) {
    ...
    default:
      if (Combat.prototype.createEmbeddedDocuments != undefined)
        unpatch(
          "Combat.prototype.createEmbeddedDocuments",
          tokensVisible.Combat._defaultcreateEmbeddedEntity
        );
      else
        unpatch(
          "Combat.prototype.createEmbeddedEntity",
  ...
  async function combatMaskedEntities(wrapped, embeddedName, data, options) {
    if (embeddedName == "Combatant") {
      ...
      if (Array.isArray(data))
        data.forEach((i) => {
          let TOKEN = canvas.scene.data.tokens.find(
            (k) => (k._id != undefined ? k._id : k.id) == i.tokenId
          );

          // break statements deliberately missing
          const disposition =
            TOKEN.disposition != undefined
              ? TOKEN.disposition
...
function unpatch(libraryID, originalfunction) {
  if (libWrapper.is_fallback)
    libWrapper.register(moduleName, libraryID, originalfunction, "OVERRIDE");
  else libWrapper.unregister(moduleName, libraryID, false);
}
